...
const { addChatUser, getChatUserByGit, updateChatUser, getUserEmoji } = require("./database");
...
async function isFullTime(id) {
    var request = await fetch(`${CONFIG.GITLAB_URL}/api/v4/groups/62/members/${id}`, {
        headers: {
...
async function verifyUser(token) {
    ...
    const user = await fetch(CONFIG.GITLAB_URL + "/api/v4/user?access_token=" + token);
...
async function getUsers() {
    ...
    while(nextPage) {
        ...
        let users = await fetch(CONFIG.GITLAB_URL + '/api/v4/users?per_page=100&active=true&humans=true&page=' + pageNumber, {
            headers: {
                Authorization: "Bearer " + CONFIG.PERSONAL_TOKEN
            }
        })
...
async function getChatUsers() {
    ...
    while (nextPage) {
        const response = await fetch(`${CONFIG.GITLAB_URL}/api/v1/users.list?offset=${offset}&count=${count}`, {
            method: 'GET',
            headers: {
...
async function getUserRepositories(id) {
    let request = await fetch(`${CONFIG.GITLAB_URL}/api/v4/users/${id}/projects`, {
        headers: {
            Authorization: "Bearer " + CONFIG.PERSONAL_TOKEN
        }
    })
...
async function getUserCommitsByDate(id, date) {
    ...
    for (let repo of repositories) {
        const request = await fetch(`${CONFIG.GITLAB_URL}/api/v4/projects/${repo.id}/repository/commits?author_id=${id}&since=${start}&until=${end}`, {
            headers: {
                Authorization: "Bearer " + CONFIG.PERSONAL_TOKEN
            }
        });
